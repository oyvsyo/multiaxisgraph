...
class MultiAxisGraph(ROOT.TObject):
    ...
    def __scale(self):
        ...
        [bottom, top] = range_y(map(lambda graph: get_graph_y(graph),
                                self.graphs["right"]))
        [y_base_min, y_base_max] = range_y(map(lambda graph:
                                           get_graph_y(graph),
                                           self.graphs["base"]))
        ...
        map(lambda i: set_graph_y(self.graphs["right_scaled"][i],
            linear_transformation(a, b, get_graph_y(self.graphs["right"][i]))),
            range(n))
